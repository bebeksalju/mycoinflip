...
<template>
    <div class="flex flex-col h-full bg-gray-900 text-xs border-t border-gray-800 font-sans relative">
        ...
        <div class="flex gap-1 px-2 border-b border-gray-800 bg-gray-950/50">
            ...
            <button @click="activeTab = 'open'" class="px-4 py-2 font-bold border-b-2 transition-colors relative"
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar bg-gray-900 relative">
                ...
                <div v-else>
                    <div v-for="order in walletStore.orderHistory" :key="order.id"
                        ...
                        <span class="text-right text-gray-300">
                            {{ (order.price || order.entryPrice || 0).toLocaleString('en-US', {
                                style: 'currency',
                            currency: 'USD'}) }}
                            <span v-if="order.exitPrice" class="text-[9px] text-gray-500 block">➜ {{
                                order.exitPrice.toLocaleString('en-US', { style: 'currency', currency: 'USD'}) }}</span>
                        </span>
                        ...
                        <span class="text-right"
                            ...
                            {{ (order.profit ? order.profit : (order.total || 0)).toLocaleString('en-US', {
                                style:
                                    'currency', currency: 'USD', signDisplay: 'always'}) }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div v-else-if="activeTab === 'open'" class="h-full flex flex-col bg-gray-900/50">
                <div v-if="walletStore.activePositions.length === 0 && walletStore.activeLimitOrders.length === 0"
                    class="flex flex-col items-center justify-center flex-1 text-gray-600 gap-3 opacity-50">
                ...
                <div v-else class="p-4 overflow-y-auto flex flex-col gap-6">
                    ...
                    <div v-if="walletStore.activeLimitOrders.length > 0">
                        <h3 class="text-xs font-bold text-gray-400 mb-3 uppercase tracking-wider">Limit Orders ({{
                            walletStore.activeLimitOrders.length }})</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div v-for="order in walletStore.activeLimitOrders" :key="order.id"
                                class="bg-gray-800 rounded-lg p-3 border border-gray-700 shadow-lg relative group">
                                <div class="flex justify-between items-start mb-2">
                                    ...
                                    <button @click="walletStore.cancelLimitOrder(order.id)"
                                        class="text-[10px] text-red-400 border border-red-900/50 bg-red-900/10 px-2 py-1 rounded hover:bg-red-900/30 transition-colors">
                                        Cancel
                                    </button>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-2 text-[11px] mb-2 bg-gray-900/50 p-2 rounded">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="font-mono text-blue-400 font-bold">{{
                                            order.limitPrice.toLocaleString('en-US', {style: 'currency', currency:
                                            'USD'}) }}</span>
                                    </div>
                                    <div class="flex flex-col text-right">
                                        <span class="text-gray-500">Current</span>
                                        <span class="font-mono"
                                            :class="store.currentPrice <= order.limitPrice && order.type === 'BUY' ? 'text-green-500' : (store.currentPrice >= order.limitPrice && order.type === 'SELL' ? 'text-green-500' : 'text-gray-400')">
                                            {{ store.currentPrice.toLocaleString('en-US', {
                                ...
                                <div class="flex justify-between items-center text-xs text-gray-400 font-mono">
                                    ...
                                    <span>Vol: {{ order.total.toLocaleString('en-US', {
                                        style: 'currency', currency:
                                        'USD'}) }}</span>
                                </div>
                            </div>
                    ...
                    <div v-if="walletStore.activePositions.length > 0">
                        <h3 class="text-xs font-bold text-gray-400 mb-3 uppercase tracking-wider">Timed Positions ({{
                            walletStore.activePositions.length }})</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div v-for="pos in walletStore.activePositions" :key="pos.id"
                                class="bg-gray-800 rounded-lg p-3 border border-gray-700 relative overflow-hidden shadow-lg">
                                <!-- Progress Bar Background -->
                                <div class="absolute bottom-0 left-0 h-1 bg-yellow-500 transition-all duration-100 ease-linear"
                                    :style="{ width: getProgress(pos.startTime, pos.endTime) + '%' }"></div>
                                ...
                                <div class="flex justify-between items-start mb-2">
                                    ...
                                    <div class="text-right">
                                        <div class="text-2xl font-mono font-bold text-white">{{
                                            getRemainingTime(pos.endTime) }}</div>
                                ...
                                <div class="grid grid-cols-2 gap-2 text-[11px] mb-2 bg-gray-900/50 p-2 rounded">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="font-mono text-gray-300">{{ pos.entryPrice.toLocaleString('en-US',
                                ...
                                <div class="flex justify-between items-center text-xs font-bold">
                                    <span class="text-gray-400">Amount: {{ pos.amount.toLocaleString('en-US', {
                                        style:
                                    ...
                                    <span :class="{
                                        ...
                                        {{ ((pos.type === 'UP' && store.currentPrice > pos.entryPrice) || (pos.type ===
                                            'DOWN' && store.currentPrice < pos.entryPrice)) ? 'WINNING' : 'lOSING' }}
                                            </span>
                                </div>
                            </div>
                        </div>
