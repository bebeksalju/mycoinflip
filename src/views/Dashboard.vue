...
<template>
    <div class="flex flex-col h-full bg-gray-950">
        ...
        <div class="md:hidden flex border-b border-gray-800 bg-gray-950 sticky top-0 z-20">
            ...
            <button 
                @click="mobileActiveTab = 'analysis'"
                class="flex-1 py-3 text-xs font-bold border-b-2 transition-colors"
        ...
        <div class="flex-1 flex flex-col md:flex-row min-h-0 overflow-hidden relative">
            ...
            >
                ...
                <div 
                    class="flex-none z-20 bg-gray-900 border-b border-gray-800 md:border-b-0 order-1 md:order-2"
                    v-show="mobileActiveTab === 'trade' || !isMobile"
                    :class="{'p-0': true}" 
                >
                    <TradeForm />
                </div>
             ...
             <div 
                ...
                class="md:hidden absolute inset-0 z-30 bg-gray-900 flex flex-col"
             >
                <OrderHistory />
            </div>
            ...
            >
                ...
                <div class="mb-4 relative">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 absolute left-3 top-3 text-gray-500">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                    </svg>
                    <input type="text" placeholder="Search markets..." class="w-full bg-gray-800 border border-gray-700 rounded-lg pl-9 pr-4 py-2.5 text-sm text-white focus:outline-none focus:border-yellow-500 placeholder-gray-600">
                </div>

                <div class="space-y-3">
                    <button 
                        v-for="coin in marketStore.supportedCoins" 
                        :key="coin"
                    ...
                    >
                        <div class="flex items-center gap-4">
                             <!-- Coin Icon/Avatar -->
                             <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-gray-900 text-sm shadow-lg shrink-0" 
                                  :class="{
                                    'bg-[#F7931A]': coin === 'BTC',
                                    'bg-[#627EEA]': coin === 'ETH',
                                    'bg-[#00FFA3]': coin === 'SOL',
                             ...
                             <div class="flex flex-col items-start gap-0.5">
                                 <span class="font-bold text-white text-base">{{ marketStore.COIN_MAP[coin].name }}</span>
                        ...
                        <div class="flex flex-col items-end gap-0.5">
                             <template v-if="getMarketData(coin)">
                                <span class="font-bold text-base tracking-wide transition-colors duration-300 font-mono">${{ getMarketData(coin).current_price?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 4 }) }}</span>
                                <span 
                                ...
                                >
                                    {{ getMarketData(coin).price_change_percentage_24h >= 0 ? '+' : '' }}{{ getMarketData(coin).price_change_percentage_24h?.toFixed(2) }}%
                                </span>
                             </template>
                             <template v-else>
