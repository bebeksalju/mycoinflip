    ...
    <div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-gray-400">
                    ...
                    <tbody class="divide-y divide-gray-700/50">
                        <tr v-for="user in authStore.mockUsers" :key="user.id" class="hover:bg-gray-700/20 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    <span class="font-bold text-white">{{ user.name }}</span>
                                    <button @click="openDetailsModal(user)" class="text-xs text-yellow-500 hover:text-yellow-400 text-left mt-0.5 hover:underline">View Details / Profile</button>
                            ...
                            <td class="px-6 py-4 font-mono text-white">
                                <div class="flex items-center gap-2">
                                    ...
                                    <button 
                                        v-if="authStore.user.role === 'superuser'"
                            ...
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 rounded-full text-xs font-bold capitalize"
                                    :class="{
                                        'bg-green-900/30 text-green-400': user.kyc === 'verified',
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="inline-flex bg-gray-900 rounded-lg p-1 border border-gray-700">
                                    <button 
                                        @click="updateProfitMode(user.id, 'loss')"
                                    ...
                                    <button 
                                        @click="updateProfitMode(user.id, 'random')"
                                        class="px-3 py-1 rounded text-xs font-bold transition-all"
                                    ...
                                    >
                                     ...
                                     <button 
                                        @click="updateProfitMode(user.id, 'win')"
                                        class="px-3 py-1 rounded text-xs font-bold transition-all"
        ...
        <div v-if="showBalanceModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div class="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-sm p-6 shadow-2xl">
                ...
                <p class="text-sm text-gray-400 mb-4">Updating balance for <span class="text-white font-bold">{{ selectedUser?.name }}</span></p>
                
                <div class="mb-6">
                    <label class="block text-xs font-bold text-gray-500 uppercase mb-1">New Balance (USD)</label>
                    <input v-model="newBalance" type="number" class="w-full bg-gray-900 border border-gray-700 rounded p-2 text-white font-mono focus:border-yellow-500 outline-none" />
                </div>

                <div class="flex gap-3">
                    <button @click="showBalanceModal = false" class="flex-1 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded font-bold">Cancel</button>
                    <button @click="confirmBalanceUpdate" class="flex-1 py-2 bg-yellow-600 hover:bg-yellow-500 text-white rounded font-bold shadow-lg">Update</button>
                </div>
            </div>
        </div>
        ...
        <!-- User Details Modal -->
         <div v-if="showDetailsModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div class="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
                <div class="flex justify-between items-start mb-6">
                    <div>
                        <h3 class="text-2xl font-bold text-white">User Details</h3>
                        <p class="text-sm text-gray-400">Full profile information for {{ selectedUser?.name }}</p>
                    </div>
                ...
                <div v-if="selectedUser" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="bg-gray-900/50 p-4 rounded-lg border border-gray-700/50">
                        ...
                        <p class="text-white font-bold text-lg">{{ selectedUser.firstName }} {{ selectedUser.lastName }}</p>
                    </div>
                    <div class="bg-gray-900/50 p-4 rounded-lg border border-gray-700/50">
                    ...
                    <div class="bg-gray-900/50 p-4 rounded-lg border border-gray-700/50">
                        <label class="block text-xs uppercase font-bold text-gray-500 mb-1">Date of Birth</label>
                        <p class="text-white">{{ selectedUser.dob || 'N/A' }}</p>
                    </div>
                    <div class="bg-gray-900/50 p-4 rounded-lg border border-gray-700/50">
                        <label class="block text-xs uppercase font-bold text-gray-500 mb-1">Place of Birth</label>
                        <p class="text-white">{{ selectedUser.pob || 'N/A' }}</p>
                    </div>

                     <div class="bg-gray-900/50 p-4 rounded-lg border border-gray-700/50">
                        <label class="block text-xs uppercase font-bold text-gray-500 mb-1">Phone Number</label>
                        <p class="text-white font-mono">{{ selectedUser.phone || 'N/A' }}</p>
                    </div>
                    
                    <div class="bg-gray-900/50 p-4 rounded-lg border border-gray-700/50 md:col-span-2">
                        <label class="block text-xs uppercase font-bold text-gray-500 mb-1">Address</label>
                        <p class="text-white">{{ selectedUser.address || 'N/A' }}</p>
                    </div>
                    
                     <div class="bg-gray-900/50 p-4 rounded-lg border border-gray-700/50 md:col-span-2 flex justify-between items-center">
                        <div>
                        ...
                        <div>
                             <label class="block text-xs uppercase font-bold text-gray-500 mb-1">Review</label>
                             <span class="px-2 py-1 rounded-full text-xs font-bold capitalize"
                                :class="{
                                    'bg-green-900/30 text-green-400': selectedUser.kyc === 'verified',
