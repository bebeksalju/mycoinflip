...
<template>
    <div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-6">
                <div class="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
                    ...
                    <div v-else class="divide-y divide-gray-700/50">
                        <div v-for="kyc in pendingKYC" :key="kyc.id" class="p-4 flex items-center justify-between hover:bg-gray-700/20 transition-colors">
                            ...
                            <button @click="openReview(kyc)" class="text-xs font-bold bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded transition-colors">
                                Review
                            </button>
                        </div>
                    </div>
                     ...
                     <div class="divide-y divide-gray-700/50">
                        <div v-for="kyc in processedKYC" :key="kyc.id" class="p-4 flex items-center justify-between">
                            <div class="flex flex-col">
                                ...
                                <span class="text-xs text-gray-600">{{ kyc.date }}</span>
                            </div>
                            <span class="text-xs font-bold uppercase px-2 py-1 rounded"
                                :class="kyc.status === 'verified' ? 'bg-green-900/30 text-green-500' : 'bg-red-900/30 text-red-500'"
                            >
                                {{ kyc.status }}
                            </span>
                        </div>
            ...
            <!-- Detail/Review Column (visible when modal open on desktop, or just side by side?) -->
            <!-- Let's use a Modal overlay actually for better focus -->
        </div>

        <!-- Review Modal -->
        <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div class="bg-gray-800 border border-gray-700 rounded-xl shadow-2xl max-w-2xl w-full flex flex-col max-h-[90vh]">
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-6">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-gray-900 p-3 rounded border border-gray-700">
                            ...
                            <p class="text-white">{{ selectedKYC.date }}</p>
                        </div>
                        <div class="bg-gray-900 p-3 rounded border border-gray-700">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="border border-gray-700 rounded-lg p-2 bg-gray-950">
                                ...
                                <div class="aspect-video bg-gray-800 rounded flex items-center justify-center text-gray-600">
                                    [Mock ID Front Image]
                                </div>
                            </div>
                            <div class="border border-gray-700 rounded-lg p-2 bg-gray-950">
                ...
                <div class="p-6 border-t border-gray-700 flex gap-4">
                    <button @click="reject" class="flex-1 py-3 bg-red-900/20 border border-red-900/50 hover:bg-red-900/40 text-red-400 font-bold rounded-lg transition-colors">
                        Reject Application
                    </button>
                    <button @click="approve" class="flex-1 py-3 bg-green-600 hover:bg-green-500 text-white font-bold rounded-lg transition-colors shadow-lg">
                        Approve & Verify
                    </button>
                </div>
            </div>
